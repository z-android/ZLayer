...
public class ZViewUtil {
    ...
    public static int mDesignDensity = 3;
    /**
    ...
    public static void setDesign(int width, int height, int density) {
        mDesignWidth = width;
        mDesignHeight = height;
        mDesignDensity = density;
    }
    ...
    public static int scaleValue(Context context, float value) {
        DisplayMetrics mDisplayMetrics = getDisplayMetrics(context);
        // 为了兼容尺寸小密度大的情况
        if (mDisplayMetrics.scaledDensity > mDesignDensity) {
            // 密度
            if (mDisplayMetrics.widthPixels > mDesignWidth) {
                value = value * (1.3f - 1.0f / mDisplayMetrics.scaledDensity);
            } else if (mDisplayMetrics.widthPixels < mDesignHeight) {
    ...
    public static int scale(int displayWidth, int displayHeight, float pxValue) {
        ...
        float scale = 1;
        try {
            float scaleWidth = (float) displayWidth / mDesignWidth;
            float scaleHeight = (float) displayHeight / mDesignHeight;
    ...
    public static void scaleContentView(ViewGroup contentView) {
        ZViewUtil.scaleView(contentView);
